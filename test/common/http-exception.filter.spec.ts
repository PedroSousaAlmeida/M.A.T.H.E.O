...
function hostWithResponse() {
  ...
  const host = {
    switchToHttp: () => ({ getResponse: () => ({ status }), getRequest: () => ({ url: '/x', method: 'GET', id: 'req-1' }) }),
...
describe('HttpExceptionFilter', () => {
  ...
  it('formats a plain HttpException', () => {
    ...
    expect(json).toHaveBeenCalledWith({ statusCode: 404, message: 'Company not found', requestId: 'req-1' });
  });
  ...
  it('keeps message and details from an object response', () => {
    ...
    expect(json).toHaveBeenCalledWith({ statusCode: 422, message: 'NFS-e rejected', details: { code: 'E1' }, requestId: 'req-1' });
  });
  ...
  it('moves class-validator message arrays into details', () => {
    ...
    expect(json).toHaveBeenCalledWith({ statusCode: 400, message: 'Validation failed', details: ['cnpj must be 14 digits', 'razaoSocial must be longer'], requestId: 'req-1' });
  });
